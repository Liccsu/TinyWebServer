    ...
    while (running_) {
        ...
        assert(buffersToWrite.empty());
        {
            ...
            if (buffers_.empty()) {
                ...
                cvBuffersEmpty_.wait_for(lock, std::chrono::seconds(flushInterval_), [&] { return !buffers_.empty(); });
            }
            // 条件发生时，将当前工作缓冲区送入 buffers 以待同步到磁盘，目的是哪怕前端在 3s
            // 内没有写满一块缓冲区，也能及时将日志同步到磁盘
            buffers_.push_back(std::move(currentBuffer_));
            // 将预分配的空闲缓冲区用作当前工作缓冲区
            currentBuffer_ = std::move(newBuffer1);
            // 直接把 buffers 里的缓冲区都拿过来，以便后续安全地访问，避免临界区过长或在访问 buffers 的时候 buffers
            // 又被前端改变
