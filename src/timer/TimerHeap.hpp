...
class TimerHeap {
    struct TimerNode {
        ...
        // std::weak_ordering：弱序，允许等价但不严格的顺序比较。不隐含可替换关系，若 a 等价于 b，则 f(a) 不一定等价于
        // f(b)
        ...
        auto operator<=>(const TimerNode& t) const { return expiration <=> t.expiration; }
    };
