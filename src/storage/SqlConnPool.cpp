...
SqlConnPool::Connection SqlConnPool::_getConnection() {
    ...
    if (connQue_.empty() && currentPoolSize_ >= maxPoolSize_) {
        ...
        cond_.wait(lock, [this]() { return !connQue_.empty() || currentPoolSize_ < maxPoolSize_; });
    }
    // 如果有空闲的连接
    if (!connQue_.empty()) {
        // 从队列中取出一个连接
...
bool SqlConnPool::ensureDatabase() const {
    ...
    // 连接不指定数据库
    if (!mysql_real_connect(tempConn, host_.c_str(), user_.c_str(), pwd_.c_str(), nullptr, port_, nullptr, 0)) {
    ...
    if (const auto error_code = mysql_errno(tempConn); error_code == ER_BAD_DB_ERROR) {
        ...
        const std::string createDBQuery =
            "CREATE DATABASE `" + dbName_ + "` CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;";
    ...
    while (!shutdown_) {
        {
            ...
            // 可中断的 60s 周期：析构时 notify_all 立即唤醒退出，避免 join 阻塞 60s
            cond_.wait_for(lock, std::chrono::seconds(60), [this] { return shutdown_; });
